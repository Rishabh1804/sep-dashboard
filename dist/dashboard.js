...
var bootState = "idle";
var unsubs = [];
var COLLS = ["production_entries", "jobs", "dft_measurements", "dispatch_events", "notes"];
var docsByColl = {};
var collErr = {};
var inflightJobs = [];
var customerNames = {};
...
function startListeners() {
  ...
  unsubs.push(fs.onSnapshot(
    ...
    (s) => {
      docsByColl.shifts = grab(s);
      delete collErr.shifts;
      paint();
    },
    onErr("shifts")
  ));
  unsubs.push(fs.onSnapshot(
    ...
    (s) => {
      docsByColl.depletions = grab(s);
      delete collErr.depletions;
      paint();
    },
    onErr("depletions")
  ));
...
function nilAlerts() {
  ...
  return Object.entries(latest).filter(([, d]) => d.level_after === 0).map(([sid, d]) => ({
