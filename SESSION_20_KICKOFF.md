...
2. **Pay rules are current; pay data is not in this repo.** The Finance tab prices wages from
   ...
   the sensitive-data rule, and `bm-role.html` (a May mock with business figures and named debtors)
   moved to `soma-internal/analysis/sep-dashboard-bm-role-2026-05.html` on the owner's word. Rates now reach a device through **Settings → Import roster**, from
