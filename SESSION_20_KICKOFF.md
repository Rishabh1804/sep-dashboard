...
2. **Pay rules are current; pay data is not in this repo.** The Finance tab prices wages from
   ...
   the sensitive-data rule, and `bm-role.html` (the May BM role and KPI page, with business figures
   and named debtors) moved to `soma-internal/analysis/sep-dashboard-bm-role-2026-05.html` on the
   owner's word. Rates now reach a device through **Settings → Import roster**, from
   ...
   until an import stamps them, Home, Finance, Production, History, Settings, both pay prints and the
   payroll and costs CSVs say the rates are *held, not imported*; after it, the prints and those CSVs
   name the card's date. A fresh install prices at zero and says so. **Whether the personal tracker keeps any pay views is this session's to
...
| **Consumes** the roster import ← `soma-internal` | **In PR #33 (alpha.16–19); not yet deployed.** Settings → Import roster reads a `sep-dashboard-roster` v1 file: `{format, version, asOf, cfg: {hourRate, permOtBaseRate, snackRate}, workers: [{id, dailyRate} or {id, monthlyWage}]}`. Workers are matched **by id**; unknown ids are skipped and counted, never created; only rate fields apply, and only the field the worker is paid by (monthly wage for a guard or plain-model worker, day rate for other monthly-tier workers, nothing for contract hands); a wrong-field or junk figure is **rejected and named**; active, non-excluded monthly-tier workers left without a figure are listed. The import stamps `cfg.rosterAsOf` from `asOf` — **even when it rejects or leaves workers unpriced**, so read the alert — so **`asOf` must change whenever any figure changes**. Row 8 in `soma-internal/docs/CROSS_REPO_SESSIONS.md` is the other side. ⚖ **`soma-internal` owns the rate card** (the private repo). This repo ships structure only. |
